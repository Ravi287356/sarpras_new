...
<div class="max-w-3xl mx-auto">
    ...
    <p class="text-slate-400 text-sm mt-1">Isi data sarpras baru</p>
    ...
    <form action="{{ route('admin.sarpras.store') }}" method="POST"
        ...
        {{-- NAMA --}}
        <div>
            <label class="text-sm text-slate-300">Nama Sarpras</label>
        ...
        <div>
            ...
            <select name="kondisi_alat_id"
                ...
                @foreach($kondisis as $k)
                    <option value="{{ $k->id }}" @selected(old('kondisi_alat_id') == $k->id)>
                        {{ $k->nama }}
                    </option>
                @endforeach
            </select>
        ...
        {{-- JUMLAH ITEM --}}
        <div>
            <label class="text-sm text-slate-300">Jumlah Item yang Ditambahkan</label>
            <input type="number" name="jumlah" id="inputJumlah" min="1"
                   value="{{ old('jumlah', 1) }}" required
                   class="mt-2 w-full rounded-xl bg-slate-950/60 text-white
                          ring-1 ring-slate-800 px-4 py-3
                          focus:outline-none focus:ring-2 focus:ring-emerald-500/60 transition">
            <p class="text-xs text-slate-500 mt-1">Jumlah item/unit yang akan dibuat dengan kode otomatis</p>
        </div>
    ...
    <p class="text-slate-400 text-sm mt-6">
        <strong>Catatan:</strong> Setiap item akan mendapatkan kode otomatis berdasarkan kategori, lokasi, dan nama sarpras.
    </p>
</div>
@endsection
