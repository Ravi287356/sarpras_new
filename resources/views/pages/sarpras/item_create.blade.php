...
<div class="max-w-3xl mx-auto">
    <h1 class="text-2xl font-semibold tracking-tight">Tambah Item: {{ $sarpras->nama }}</h1>
    <p class="text-slate-400 text-sm mt-1">Tambahkan unit/item baru untuk sarpras ini</p>
    ...
    <form action="{{ route('admin.sarpras_item.store', $sarpras->id) }}" method="POST"
          class="mt-6 rounded-2xl bg-slate-900/40 ring-1 ring-slate-800 p-6 space-y-5">
        @csrf

        {{-- KODE --}}
        <div>
            <label class="text-sm text-slate-300">Kode Item (Unique)</label>
        ...
        <div>
            ...
            <select name="status_peminjaman_id"
                ...
                @foreach($statuses as $s)
                    <option value="{{ $s->id }}" @selected(old('status_peminjaman_id') == $s->id)>
                        {{ $s->nama }}
                    </option>
                @endforeach
            </select>
