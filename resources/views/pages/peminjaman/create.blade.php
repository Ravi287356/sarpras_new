    ...
    <div class="rounded-2xl border border-white/10 bg-white/5 p-6">
        ...
        <form action="{{ route('user.peminjaman.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                @error('jumlah') <div class="text-rose-300 text-sm mt-1">{{ $message }}</div> @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="tanggal_pinjam" value="{{ old('tanggal_pinjam', date('Y-m-d')) }}"
                           class="mt-2 w-full rounded-xl bg-slate-950/40 border border-white/10 px-4 py-3 text-slate-100 outline-none">
                    @error('tanggal_pinjam') <div class="text-rose-300 text-sm mt-1">{{ $message }}</div> @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_kembali" value="{{ old('tanggal_kembali') }}"
                           class="mt-2 w-full rounded-xl bg-slate-950/40 border border-white/10 px-4 py-3 text-slate-100 outline-none">
                    @error('tanggal_kembali') <div class="text-rose-300 text-sm mt-1">{{ $message }}</div> @enderror
                </div>
            </div>
