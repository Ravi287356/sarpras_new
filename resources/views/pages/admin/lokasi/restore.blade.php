...
<div class="max-w-6xl">
    ...
    <div class="flex items-start justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-semibold text-white">📍 Lokasi Terhapus</h1>
            <p class="text-slate-400 text-sm mt-1">Daftar lokasi yang telah dihapus dan bisa dipulihkan</p>
        </div>
                ...
                @foreach ($items as $i => $u)
                    <tr class="hover:bg-slate-950/30 transition text-slate-200">
                        ...
                        <td class="px-6 py-4 text-center">
                            <form action="{{ route('admin.lokasi.restore', $u->id) }}" method="POST">
                                @csrf
                                <button type="submit"
                ...
                @if($items->count() === 0)
                    <tr>
                        <td colspan="4" class="px-6 py-10 text-center text-slate-500 italic">
                            Tidak ada lokasi yang terhapus.
                        </td>
                    </tr>
                @endif
            </tbody>
