...
<div class="max-w-6xl">
    ...
    <div class="flex items-start justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-semibold text-white">📑 Kategori Sarpras Terhapus</h1>
            <p class="text-slate-400 text-sm mt-1">Daftar kategori yang telah dihapus dan bisa dipulihkan</p>
        </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-slate-950/40 text-slate-200">
                <tr class="text-left">
                    ...
                    <th class="px-6 py-4 text-slate-300">Dihapus Pada</th>
                    <th class="px-6 py-4 w-40 text-center">Aksi</th>
                </tr>
            </thead>

            <tbody class="divide-y divide-slate-800">
                @foreach ($categories as $i => $u)
                    <tr class="hover:bg-slate-950/30 transition">
                        <td class="px-6 py-4 text-slate-300">{{ $i + 1 }}</td>
                        <td class="px-6 py-4 font-semibold text-white">{{ $u->nama }}</td>
                        <td class="px-6 py-4 text-slate-400">
                            {{ $u->deleted_at?->format('d-m-Y H:i') }}
                        </td>
                        <td class="px-6 py-4 text-center">
                            <form action="{{ route('admin.kategori_sarpras.restore', $u->id) }}" method="POST">
                                @csrf
                                <button type="submit"
